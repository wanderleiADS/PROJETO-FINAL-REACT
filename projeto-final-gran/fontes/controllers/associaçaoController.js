...
exports.associarFornecedor = (req, res) => {
  ...
  db.query(verificarAssociacao, [produtoId, fornecedorId], (err, results) => {
    ...
    if (results.length > 0) {
      return res
        .status(400)
        .json({ mensagem: "Fornecedor já está associado a este produto!" });
    }
    ...
    db.query(sql, [produtoId, fornecedorId], (err) => {
      if (err)
        return res
          .status(500)
          .json({ mensagem: "Erro ao associar fornecedor.", erro: err });
  ...
  db.query(sql, [produtoId], (err, results) => {
    if (err)
      return res.status(500).json({
        mensagem: "Erro ao listar fornecedores associados.",
        erro: err,
      });
...
exports.desassociarFornecedor = (req, res) => {
  ...
  db.query(sql, [produtoId, fornecedorId], (err) => {
    if (err)
      return res
        .status(500)
        .json({ mensagem: "Erro ao desassociar fornecedor.", erro: err });
